...
export default function HostChatList({ usersList, onlineUsers, lastMessage, setChatClicked, chatClicked }) {
  // console.log(usersList,'usersList');
  const [users, setUsers] = useState([])
  useEffect(() => {
    if (usersList) {
      console.log(usersList,"usedeedrsList");
      setUsers(usersList)
    }
  }, [usersList])
  const searchUsers = (e) => {
          ...
          <ul className="list-unstyled mb-0">

            {
              ...
              users.map((chat, index) => {
                console.log(chat,"chawddt");
                return (
                  <li className="p-2 border-bottom">
                    <Link to={"/host/chat?id=" + chat.userId._id} onClick={() => setChatClicked(true)} className="d-flex justify-content-between">
