...
import { addHostMessage, getHostMessages } from '../../../api/hostMessageRequests';

export default function HostMessageList({ currentChat, chatClicked, setSendMessage, receivedMessage }) {
  ...
  const sentMessage = async (e) => {
    ...
    setSendLoading(true)
    try {
      const { data } = await addHostMessage({
  ...
  useEffect(() => {
    try {
      (async function () {
        const { data } = await getHostMessages(currentChat._id)
        if (!data.err)
          ...
          <>
            <div className="row ps-2 d-flex align-items-center message-head" style={{ height: "45px" }}>
              <div className='d-flex align-items-center' style={{ gap: "10px" }} >
                <i className="fa-sharp fa-solid fa-arrow-left-long" onClick={()=>navigate("/account/host/chat")}  />
                {/* <Avatar alt="Remy Sharp" sx={{ width: 32, height: 32 }} src={currentChat.userId.picture && currentChat.userId.picture} /> */}
                <b className="ps-1">{currentChat.userId.firstName}</b>
              </div>
            </div>
